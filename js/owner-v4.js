...
function getProfile(){
  try{const data=JSON.parse(localStorage.getItem(SAVE_KEY)||'{}');return normalize(data.profile)}catch{return normalize(null)}
}
function saveProfile(profile){
  try{const data=JSON.parse(localStorage.getItem(SAVE_KEY)||'{}');data.profile=profile;data.version='4.0.0';localStorage.setItem(SAVE_KEY,JSON.stringify(data))}catch(e){console.error('Owner save failed',e)}
}
function toast(message){const e=document.getElementById('toast');if(e){e.textContent=message;e.classList.add('show');setTimeout(()=>e.classList.remove('show'),2200)}else alert(message)}
...
function tool(id){
 ...
 saveProfile(p);if(window.goHome)window.goHome();
}
function open(){
 ...
 document.body.appendChild(overlay);overlay.querySelectorAll('[data-owner]').forEach(b=>b.onclick=()=>tool(b.dataset.owner));overlay.querySelector('#ownerClose').onclick=open;
}
function mountButton(){
 if(document.getElementById('ownerLauncher'))return;
 const b=document.createElement('button');b.id='ownerLauncher';b.textContent='👑 OWNER';b.title='Owner Panel (O)';b.onclick=open;
 b.style.cssText='position:fixed;right:18px;bottom:18px;z-index:9998;padding:10px 14px;border-radius:10px;border:1px solid rgba(255,255,255,.2);background:#111827;color:#fff;font-weight:700;cursor:pointer;box-shadow:0 6px 20px rgba(0,0,0,.35)';document.body.appendChild(b)
}
